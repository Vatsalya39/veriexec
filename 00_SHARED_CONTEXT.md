...
│   ├── scenarios.json        # §10
│   ├── CRYPTO_WIRE_FORMAT.md # signature + public-key encoding, signed bytes, one frozen vector (B+C, G0)
│   ├── CANONICAL_JSON_VECTORS.json  # dict → canonical string → sha256, shared by B's fingerprint and C's chain
...
├── packages/bench/           # TEAM C ONLY (the 22-fixture harness, `make bench`)
├── apps/console/             # TEAM C ONLY (React + Vite + TS)
├── services/audit/           # TEAM C ONLY (FastAPI: audit chain, chatbot, canary, bench)
├── scripts/verify_chain.py   # shared — verifies an exported audit log with no service running
├── tests/conformance/        # shared — one test per Invariant, all teams run it
└── docs/
    ├── OWNERSHIP.md          # shared — every file, with its owning team. Appended to, never rewritten.
    ├── STYLOMETRY.md         # A
    ├── RISK_WEIGHTS.md       # B
    ├── THREAT_MODEL.md       # C, reviewed by B
    ├── BENCH_NOTES.md        # C — the benchmark's limitations, written before the results are known
    ├── RESULTS.md            # generated by `make bench` — the five metrics and the matrix
    ├── DEMO_SCRIPT.md        # C
...
|---|---|---|
| A — signal | `8001` | `POST /v1/process-communication`, `POST /v1/extract`, `POST /v1/analyze-signals`, `POST /v1/freshness/issue`, `GET /v1/samples`, `GET /v1/samples/{id}`, `GET /healthz` |
| B — core | `8002` | `POST /v1/assess-risk`, `POST /v1/fingerprint`, `POST /v1/fingerprint/verify`, `POST /v1/challenge/issue`, `POST /v1/challenge/validate`, `POST /v1/device/enrol`, `POST /v1/signature/verify`, `POST /v1/token/mint`, `POST /v1/token/redeem`, `GET /v1/breaker`, `POST /v1/breaker/close`, `POST /v1/replay`, `POST /v1/mode`, `GET /v1/policy`, `GET /v1/explain/{transaction_id}`, `GET /healthz` |
| C — audit | `8003` | `POST /v1/audit/append`, `GET /v1/audit/head`, `GET /v1/audit/verify`, `GET /v1/audit/records`, `GET /v1/audit/export`, `POST /v1/audit/ask`, `POST /v1/audit/_tamper` *(demo only)*, `POST /v1/canary/run`, `POST /v1/bench/run`, `GET /healthz` |
| C — console | `5173` | Vite dev server. Routes: `/`, `/tx/:id`, `/device`, `/desk`, `/timeline`, `/audit`, `/canary`, `/benchmark`, `/sandbox` |
...
| `INTENTLOCK_HMAC_SECRET` | `dev-only-not-a-secret` | Capability-token and duress-marker MAC key. The literal default is allowed **only** when `INTENTLOCK_MODE != production`; a startup guard logs `WARNING: using the development HMAC secret` on every boot and refuses to start if `INTENTLOCK_MODE=production` and the value is still the default. A silent default in a production build is the vulnerability. |
| `INTENTLOCK_DEMO_ENDPOINTS` | *(unset)* | `1` registers the demo-only routes (`POST /v1/audit/_tamper`). When unset those routes **do not exist** — a `404`, not a `403`. Never set in anything but a demo run. |
