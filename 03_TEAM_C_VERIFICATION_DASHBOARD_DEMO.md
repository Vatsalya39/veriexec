...
and asserts the serialized DOM contains neither `"4,50,000"`, `"450000"`, nor the last-4 of the
...
`expires_at`. On submit, POST all three answers together to B's `/v1/challenge/verify` — never
...
failure is that the callback becomes a conversation the attacker can steer: *"yes, yes, the ₹45 lakh
...
`/v1/breaker/state` every 5 s so it cannot go stale mid-demo.
...
`/v1/assess`. Every input a judge would want to twist, exposed as a control:
...
`packages/bench/run_bench.py` loads all 22 golden fixtures, calls B's `/v1/assess` on each, and
...
| --- | --- | --- |
| Attack detection rate | `blocked_attacks / total_attacks` (attacks = the 11 malicious fixtures) | ≥ 10/11 |
| False positive rate | `blocked_or_challenged_legit / total_legit` (11 benign fixtures) | ≤ 2/11 |
| Median decision latency | p50 over 22 runs, excluding LLM (and a second figure including it) | < 900 ms |
| Explanation completeness | fraction of decisions where every non-zero contribution carries a resolvable `evidence_ref` | 100% |
| Abstention correctness | fraction of `unavailable` dimensions that produced a coverage penalty rather than a zero | 100% |

Render all five in the `/benchmark` screen with the **raw fractions next to the percentages**.
`10/11 (90.9%)` is credible; `90.9%` alone invites *"out of how many?"* — and you want the judge to
notice the denominator is small before they ask, because volunteering the limitation is what makes the
rest believable.
...
| 9 | 5:00–5:40 | **The numbers** | Benchmark screen: 10/11, 1/11, p50, sweep chart | *"Twenty-two scenarios we wrote ourselves. Treat it as a smoke test, not an evaluation — and here is the threshold sweep that shows we chose 70 rather than guessed it."* |
...
| `GET` | `/v1/health` | `{ok, chain_ok, record_count, mode, version}` — every service exposes this identically. |
...
|---|---|---|
| B `:8002` | `POST /v1/assess` | The whole verification panel and the sandbox. |
| B `:8002` | `POST /v1/challenge/issue` · `/verify` | §9. |
| B `:8002` | `POST /v1/device/enrol` · `/v1/signature/verify` | §10. |
| B `:8002` | `POST /v1/token/redeem` | Final execution step after PASS + signature. |
| B `:8002` | `GET /v1/breaker/state` · `POST /v1/mode` | §14, §20. |
| B `:8002` | `POST /v1/replay` | §21.2 replay button. |
| B `:8002` | `GET /v1/policy` | `policy_version` + `policy_hash` in the footer and sandbox chip. |
