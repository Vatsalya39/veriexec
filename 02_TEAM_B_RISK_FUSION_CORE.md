...
| `POST` | `/v1/device/enrol` | `{device_id, public_key_spki_b64u}` ⇒ `{device_id, key_fingerprint}` |
...
| `POST` | `/v1/mode` | `{mode}` ⇒ set `FULL\|NO_LLM\|NO_DETECTORS\|MINIMAL`. Server-side, audited as `MODE_CHANGED`. Powers Team C's kill switch. |
