/**
 * `copy/reasons.ts` — every raw enum mapped to a sentence, in one file, so the language
 * is consistent everywhere and reviewable in one place. §8.3: never render a raw enum.
 */
...
const REASONS: Record<string, string> = {
  ...
  FC-2: "A detector received input it could not score — scored as unavailable, not as clean.",
...
/** A sentence for a code. Unknown codes render as the code itself — never a blank. */
export function reasonSentence(code: string | null | undefined): string | null {
  if (!code) return null;
  return REASONS[code] ?? code;
}

/** The ladder line for the step rail: friction that explains itself. §15 */
